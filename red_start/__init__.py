...
def copy_template(src, dest, replace=None):
    ...
    for path, dirs, files in os.walk(src):
        ...
        for filename in files:
            if (filename.startswith('red_start') or
...
def start_project(copy_to, copy_from=None, no_prompt=False):
    if not copy_from:
        copy_from = os.path.abspath(os.path.join(os.path.dirname(__file__),
                                    'templates', 'project', 'ff0000'))

    shutil.copytree(copy_from, copy_to, ignore=shutil.ignore_patterns('red_start*',))
